...
const PhotoUploadWidget: React.FC<IProps> = ({loading, uploadPhoto}) => {
  ...
  useEffect(() => {
    // clear up the object URL from the previous render ( not just on unmount ) to prevent 
    // memory leaks
    return () => {
          ...
          {files.length > 0 &&
            <Fragment>
              ...
              <Button.Group widths={2}>
                <Button positive icon='check' loading={loading} onClick={uploadPhoto.bind(null, image!)}/>
